...
// ✅ CORRIGÉ : JWT sécurisé au lieu de userId:timestamp forgeable
...
function makeToken(userId: number): string {
  ...
  const payload = { sub: userId, type: "access", iat: now, exp: now + 7 * 24 * 60 * 60 };
...
const conversationOpsRouter = opsRouter({
  ...
  sendMessage: protectedOpsProcedure.input(z.object({
    ...
    // ✅ KOSSI mode agent autonome avec Tool Calling Loop (web search, calculate, etc.)
    const kossiSystemPrompt = `${systemPrompt}

${AGENT_TOOLS_PROMPT}`;
    const allMsgs: LLMMessage[] = [{ role: "system", content: kossiSystemPrompt }, ...historyMessages];
    let assistantContent: string;
    try {
      const agentResult = await runAgentLoop(
        allMsgs,
        async (msgs) => {
          const r = await invokeLLM({ messages: msgs, max_completion_tokens: 800 });
      ...
      const response = await invokeLLM({ messages: allMsgs });
